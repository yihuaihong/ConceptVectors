...
def collect_logits(model, input_ids, layers=[46, 47]):
    ...
    # max_values, _ = torch.max(decoder_out, dim=-1)
    # print(max_values)
...
def make_decoder(model, decoder_layer_names=['final_layernorm', 'lm_head']):
    ...
    if 'olmo' in model.config._name_or_path.lower():
        decoder_layer_names=['model.transformer.ff_out']
    # print(dir(model))
...
def final_layernorm_locator(model: nn.Module):
    ...
    if 'llama' in model.config._name_or_path.lower():
        layernorms_on_base_model = [
            mod for mod in model.base_model.children()
            if isinstance(mod, LlamaRMSNorm)
            # if isinstance(mod, nn.LayerNorm)
        ]
    else:
        layernorms_on_base_model = [
            mod for mod in model.base_model.children()
            if isinstance(mod, OlmoLayerNorm)
        ]
...
def _get_layer(model, name):
    ...
    model_with_module = model if name == "lm_head" or name == "model.transformer.ff_out" else model.base_model
...
def compute_z(
    ...
    try:
        ...
        if 'olmo' in model.name_or_path.lower():
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            if args.entropy_loss:
                ...
                #print('layer_logits.shape1: ',layer_logits.shape)
                last_nonzero_mask = torch.remainder(torch.argmin(input_tok["attention_mask"], -1) - 1,
                                                    input_tok["attention_mask"].shape[-1])
                ...
                layer_logits = F.softmax(layer_logits, dim=-1) #bug here
                                                               #also bug here, why always 4.844???
        ...
        if args.fact_erasure:
            ...
            if args.margin_loss:
                ...
                pred_prob = torch.exp(-nll_loss)  # 这个就是fact erasure，想minimize那个概率
                loss = pred_prob + kl_loss + weight_decay
                print(
                    f"loss {np.round(loss.item(), 3)} = {np.round(pred_prob.item(), 3)} + {np.round(kl_loss.item(), 3)} + {np.round(weight_decay.item(), 3)} "
                    f"avg prob of [{request['target_new']['str']}] "
                    f"{torch.exp(-nll_loss_each).mean().item()}"  #这里就是想把对应的probability降到最低
                )
        else:
